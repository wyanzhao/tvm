...
// Positional relay function to create conv2d operator
// used by frontend FFI.
Expr MakeNvdlaConv2DBias(Expr data,
                Expr weight,
                Expr bias,
                Array<IndexExpr> strides,
                Array<IndexExpr> padding,
  ...
  attrs->out_layout = std::move(out_layout);
  attrs->out_dtype = std::move(out_dtype);
  static const Op& op = Op::Get("nn.nvdla_conv2d_bias");
  return CallNode::make(op, {data, weight, bias}, Attrs(attrs), {});
}

// Positional relay function to create conv2d operator
// used by frontend FFI.
Expr MakeNvdlaConv2D(Expr data,
                Expr weight,
                Array<IndexExpr> strides,
                Array<IndexExpr> padding,
                Array<IndexExpr> dilation,
  ...
  attrs->out_layout = std::move(out_layout);
  attrs->out_dtype = std::move(out_dtype);
  static const Op& op = Op::Get("nn.nvdla_conv2d");
  return CallNode::make(op, {data, weight}, Attrs(attrs), {});
}


TVM_REGISTER_API("relay.op.nn._make.nvdla_conv2d")
.set_body_typed(MakeNvdlaConv2D);


RELAY_REGISTER_OP("nn.nvdla_conv2d")
.describe(R"code(2D convolution layer (e.g. spatial convolution over images).
...
TVM_REGISTER_API("relay.op.nn._make.nvdla_conv2d_bias")
.set_body_typed(MakeNvdlaConv2DBias);

RELAY_REGISTER_OP("nn.nvdla_conv2d_bias")
.describe(R"code(2D convolution layer (e.g. spatial convolution over images).
...
.set_support_level(2)
.add_type_rel("NvdlaConv2D", NvdlaConv2DBiasRel<Conv2DAttrs>)
.set_attr<FInferCorrectLayout>("FInferCorrectLayout", Conv2DInferCorrectLayout<Conv2DAttrs>);
