...
dtype="float32"
data_shape = (1, 1, 3, 4)
weight_shape = (9, 1, 1, 1)
x = relay.var("x",
                     shape=data_shape,
                     dtype=dtype)
w = relay.var("w", shape=(9, 1, 1, 1), dtype="float32")
network = relay.nn.conv2d(x, w, strides=(1, 1,), padding=(0, 0), dilation=(1, 1), groups=1, channels=9, data_layout="NCHW")
...
#kernel = np.random.uniform(size=weight_shape).astype(dtype)
kernel = np.array([
    [
        [
            [
