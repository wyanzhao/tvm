...
dtype="float32"
data_shape = (1, 1, 1, 1)
weight_shape = (15, 1, 1, 1)
x = relay.var("x",
                     shape=data_shape,
                     dtype=dtype)
w = relay.var("w", shape=weight_shape, dtype=dtype)

network = relay.nn.conv2d(x, w, strides=(1, 1,), padding=(0, 0), dilation=(1, 1), groups=1, channels=15, data_layout="NCHW")
network = relay.nn.softmax(network)
